...
class Conformal_nxg():
    ...
    """
    def __init__(self, forget_factor, input_factor, num_input_vars):
    ...
    def calibrate(self, data, forecast, label):
        ...
        if self.cs.size != 0:
            ...
            for i in range(self.num_input_vars):
                variance[:,i] = np.std(input_vars[i],axis=1)
            self.cs = np.r_[self.cs,  ((1 + variance @ self.input_factor.T) *np.abs(label-forecast))]
        else:
            ...
            self.cs = (1+ variance @ self.input_factor.T) * np.abs(label-forecast)
    ...
    def predict(self,data,forecast, length_distr = 200, ymin = 0, ymax = 100):
        ...
        variance = np.std(np.split(data,self.num_input_vars),axis=1)
        ...
        for i in range(num_i - 1):
            ...
            if(np.sum(weights_cal) >= confidence):
                ...
                cal_thres = (np.sort(self.cs)[ind_thres])/ (1 + variance @ self.input_factor.T)
                if cal_thres < 0:
